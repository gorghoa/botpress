...
class FlowBuilder extends Component<Props, State> {
  ...
  handleFilterChanged = event => {
    ...
    const newUrl = this.props.location.pathname + searchTag + val
    this.setState({ highlightFilter: val })
    this.props.history.replace(newUrl)
  }

  pushFlowState = (flow) => {
    let hash = ''
    if (this.state.showSearch) hash = searchTag + this.state.highlightFilter
    ...
    const keyHandlers = {
      ...
      find: e => {
        ...
        if (this.state.showSearch) {
          this.props.history.replace(this.props.location.pathname + searchTag + this.state.highlightFilter)
        }
      },
      'preview-flow': e => {
        e.preventDefault()
